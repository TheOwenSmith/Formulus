# PhoenixTrader — Claude Code Guide
...
```ts
import { fromThrowable, fromThrowableAsync, internal, badRequest } from '@api/utils/error-handling';
...
**AppError type**:
```ts
type AppError = { code: TRPC_ERROR_CODE_KEY; message?: string; error?: unknown }
```
...
- **Mutations** — use `onError` callback in `mutationOptions`:
  ```tsx
  useMutation(trpcCredentials.users.updateProfile.mutationOptions({
    onError: (error) => {
...
useMutation(trpcCredentials.path.to.query.mutationOptions({ onError, onSuccess }))
```
