    ...
    <header className="bg-slate-900/80 backdrop-blur-[10px] border-b border-white/10 sticky top-0 z-50 shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
      <nav className="max-w-[1400px] mx-auto px-8 py-5 flex items-center justify-between">
        <Link to="/backtest" className="no-underline transition-all duration-300 flex items-center">
          <img src={logoWide} alt="PhoenixTrader" className="h-14 w-auto" />
        </Link>
        <div className="flex items-center gap-2">
          <NavLink
            to="/backtest"
            className={({ isActive }) =>
              `text-sm font-medium px-5 py-2.5 rounded-lg transition-all duration-300 relative no-underline tracking-[0.01em] border ${
                isActive
